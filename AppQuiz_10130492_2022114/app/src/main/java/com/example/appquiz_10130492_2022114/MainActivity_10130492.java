...
public class MainActivity_10130492 extends AppCompatActivity {
    ...
    private void getData(){
        ...
        String url = "https://cloud.culture.tw/frontsite/trans/emapOpenDataAction.do?method=exportEmapJson&typeId=K";
    ...
    private void parseJson(JSONArray jsonArray){
        try{
            ...
            for(int i = 0 ; i < jsonArray.length() ; i++){
                ...
                String openTime = jsonObject.getString("closeDay");
                BookStore bookStore = new BookStore(name,cityName,openTime);
                Log.d("JSONData", String.format("Name: %s, City: %s, CloseDay: %s",bookStore.getName(),bookStore.getCityName(), bookStore.getCloseDay()));
                bookStores.add(bookStore);
            }

            Log.i("JSON", bookStores.toString());
            adapter.notifyDataSetChanged();
